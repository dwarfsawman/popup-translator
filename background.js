...
chrome.contextMenus.onClicked.addListener((info, tab) => {
  if (info.menuItemId === "translate-with-OpenRouter") {
    chrome.tabs.sendMessage(tab.id, {
      action: "showPopupFromContextMenu",
...
async function translateTextWithOpenRouter(text, targetLanguage, apiKey, model) {
  ...
  switch (targetLanguage) {
    case "Japanese":
      systemPrompt =
        "以下の文章を日本語訳してください。なるべく直訳は避け自然な日本語にしてください。前置きや説明は省き、翻訳結果だけを出力してください。";
      break;
    case "English":
      systemPrompt =
        "Please translate the following text to English. Make it natural and avoid literal translation. Output only the translation without any preamble or explanation.";
      break;
    case "Korean":
      systemPrompt =
        "다음 문장을 한국어로 번역해주세요. 직역보다는 자연스러운 한국어로 번역해주세요. 전제나 설명 없이 번역 결과만 출력해주세요.";
      break;
    case "Chinese":
      systemPrompt =
        "请将以下文本翻译成中文。请避免直译，使用自然的中文表达。 只输出翻译结果，不要任何前言或解释。";
      break;
    default:
  ...
  try {
    ...
    return {
      error: `ネットワークエラーまたはリクエスト失敗: ${error.message}`,
    };
  }
}
